...
import { turnToListOfPairs } from "../Utils";
...
const Home: NextPage<IHomepageProps> = ({
  ...
  const [listOfNumberDescPairs, setListOfNumberDescPairs] = useState(
    turnToListOfPairs(initialTradingPairData)
  );
  ...
  const handleSelectedPair = async (pair: IPair) => {
    try {
      ...
      const listOfNumDescPair = turnToListOfPairs(pairData);
      setListOfNumberDescPairs(listOfNumDescPair);
          ...
          <div className="w-full overflow-hidden lg:my-2 lg:w-full xl:my-1 xl:px-1 xl:w-1/2">
            {loading ? (
              ...
              <GJNumbersView
                ...
                listOfNumberDescPairs={listOfNumberDescPairs}
              />
            )}
          </div>
        </div>
